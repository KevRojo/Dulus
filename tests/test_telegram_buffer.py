...
def test_telegram_buffer_pruning():
    ...
    dulus_input._output_buffer.clear()
    dulus_input.append_output("  📩 Telegram: Hello 1")
    dulus_input.append_output("Some model output")
    dulus_input.append_output("  📩 Telegram: Hello 2")
    dulus_input.append_output("More model output")
    dulus_input.append_output("  📩 Telegram: Hello 3")
    
    # Before pruning: 5 lines
    assert len(dulus_input._output_buffer) == 5
    ...
    if len(_tg_idx) > 1:
        _drop = set(_tg_idx[:-1])
        dulus_input._output_buffer[:] = [ln for i, ln in enumerate(dulus_input._output_buffer) if i not in _drop]
    
    # After pruning: 3 lines (Hello 1 and 2 removed, keep Hello 3)
    assert len(dulus_input._output_buffer) == 3
    assert "Hello 3" in dulus_input._output_buffer[-1]
    assert "Some model output" in dulus_input._output_buffer[0]
    assert "More model output" in dulus_input._output_buffer[1]
