...
"""
    ...
    try:
        ...
        generation_system = (
            ...
            "BEFORE GENERATING — CHECK THE SKILL CATALOG.\n"
            "If `~/.dulus/skills_catalog.txt` exists (tab-separated source\\tid\\tdescription),\n"
            "grep it for keywords related to the repo's domain. If an existing awesome/composio/local\n"
            "skill already covers the same functionality cleanly, MENTION IT in the plugin's NOTES\n"
            "block so the user knows there's a lighter alternative. Generate the plugin anyway — but\n"
            ...
            "4. Verify with `dulus -c \"plugin list\"` and `dulus -c \"schema <ToolName>\"` per tool.\n"
            "Surface these four steps in the plugin's README or a final NOTES block — many users miss them and think the plugin is broken when it's just unregistered.\n\n"
            "IF THE SOURCE REPO IS AN MCP SERVER — DO NOT INTEGRATE AS MCP.\n"
            "Detection signals (any of these means the repo is an MCP server):\n"
            "  • imports `mcp`, `mcp.server`, `mcp.server.fastmcp`, `FastMCP`, `@modelcontextprotocol/sdk`\n"
            "  • files named `mcp_server.py`, `server.py` exposing `@mcp.tool()` decorators, or any `.bat`/`.sh`\n"
            "    launcher that spawns the package as a stdio MCP server\n"
            "  • `pyproject.toml`/`package.json` mentions an `mcp-*` script or entry point\n"
            "  • README that talks about 'add this to claude_desktop_config.json' or `mcpServers`\n\n"
            "Why we bypass MCP here: Dulus runs tools natively in-process via its own ToolDef registry.\n"
            "Going through MCP would add a subprocess + stdio handshake per call — slower, harder to debug,\n"
            "and the agent loses the ability to introspect/cancel mid-call. We always prefer native tools.\n\n"
            "When the repo is an MCP server, your job is to UNWRAP it:\n"
            "  1. Open the MCP server file (e.g. `mcp_server.py`) and read every `@mcp.tool()` /\n"
            "     `@server.list_tools()` / `@server.call_tool()` registration.\n"
            "  2. For each registered tool, locate the underlying plain Python function (it is almost always\n"
            "     in a sibling module like `client.py`, `api.py`, `core.py`, or `<pkg>/__init__.py` — the\n"
            "     MCP handler is usually a 5-line wrapper that just calls it and JSON-encodes the result).\n"
            "  3. Build TOOL_DEFS that call those underlying functions DIRECTLY. Translate the MCP tool's\n"
            "     `inputSchema` into your JSON Schema with the same property names so the agent UX matches\n"
            "     what the MCP version offered.\n"
            "  4. Drop every MCP-specific import (`from mcp.server ...`, `import mcp`, decorators).\n"
            "     The generated `plugin_tool.py` MUST be importable without the `mcp` package installed.\n"
            "  5. If a tool truly only exists as an MCP handler (no separable Python function), inline its\n"
            "     logic into a plain function inside `plugin_tool.py` and call THAT — never spawn the MCP\n"
            "     server as a subprocess from inside the tool.\n"
            "  6. Note in the plugin README/NOTES block: 'Adapted from MCP server <original_name>. Tools\n"
            "     are exposed natively in Dulus — no MCP transport required.'\n\n"
