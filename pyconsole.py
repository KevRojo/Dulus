...
Design (why a subprocess, not in-process exec):
  * Isolation. Arbitrary model-written code runs in loops; an infinite loop,
    `os._exit()`, or a segfault kills the KERNEL, never Dulus. The parent detects
  ...
  * Clean timeouts. The parent enforces a wall-clock timeout by waiting on a
    ...
    of an infinite loop) and Dulus keeps going.
...
stdlib-only on purpose: the worker process must not drag in the Dulus runtime, so
...
class _PyKernel:
    """Manages one persistent worker subprocess for the whole Dulus session.
...
# One kernel per Dulus process (sub-agents share it; the lock keeps it safe).
_KERNEL = _PyKernel()
...
_DESCRIPTION = (
    ...
    "a crash cannot take Dulus down.\n\n"
...
if __name__ == "__main__" and "--pykernel-worker" in sys.argv:
    _worker_main()
